...
def community_detail(request, community_name):
    ...
    def get_community_detail(community_name):
        """
    ...
    literature_and_arts = list(map(get_community_detail, ['literatures and arts']))[0]
    context["literature_and_arts"] = literature_and_arts

    # Query course materials
    course_materials = list(map(get_community_detail, ['course materials']))[0]
    context["course_materials"] = course_materials

    # Query teaching materials
    teaching_materials = list(map(get_community_detail, ['teaching materials']))[0]
    context["teaching_materials"] = teaching_materials

    # Query reference materials detail
    reference_materials = list(map(get_community_detail, ['reference materials']))[0]
    context["reference_materials"] = reference_materials

    # Query newpapers and magazines detail
    newpapers_and_magazines = list(map(get_community_detail, ['newspaper and magazines']))[0]
    context["newpapers_and_magazines"] = newpapers_and_magazines

    # Query newpapers and magazines detail
    othereducational_materials = list(map(get_community_detail, ['other educational materials']))[0]
